...
- ```main``` is the current branch which contains the fully functional and verified pipelined processor with hazard handling, 2-way cache and branch prediction (in the ```rtl``` folder). To run this, the newest version of Verilator ***MUST*** be downloaded (Version 5.031), otherwise it will not function as intended. There are verilator errors that stop the cache from being built in the verilator version we installed in Lab0. This design choice was made unintentionally as Lucas, responsible for the design of the cache, had to install this version of verilator earlier on in the academic year 
